...
#include "word.h"  
    ...
    void List::insert( const Word foo )
    {
        ...
        Node * temp = new Node ( curr->prev, foo, curr );
    ...
    void List::print ( ostream & ost ) const
    {
	    ...
	    while ( curr != &head )
	    {
            ...
            cout << pos << " : " << curr->data << endl;
	        curr = curr->next;
            ++pos;
	    }
    }
    
    Word & List::operator [] ( const int n ) const
    {    
        Node * curr = head.next;
        int pos = 1;
        while ( curr != &head && pos < n )
        {
            curr = curr->next;
            ++pos;
        }

        static Word rval_ptr = Word("");
        // this may not be the nth spot
        if ( curr != &head && pos == n )
            rval_ptr = curr->data;
