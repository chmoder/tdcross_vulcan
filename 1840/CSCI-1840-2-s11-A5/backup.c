...
struct DB *openDb(char *name)
{
    ...
    FILE *file;
    struct DB *db = newDB();
    struct Srec *orig = newSrec();
    struct Srec *lname = newSrec();
    struct Srec *fname = newSrec();
    struct Srec *sid = newSrec();
    struct Srec *gpa = newSrec();
    ...
    if(file_len)
    {
        ...
        {
            fread(orig, sizeof(struct Srec), 1, file);

            copySrec(lname, orig);
            insertNode(&(db->lname_db), lname, lname_cmp);

            copySrec(fname, orig);
            insertNode(&(db->fname_db), fname, fname_cmp);
            
            copySrec(sid, orig);
            insertNode(&(db->sid_db), sid, sid_cmp);

            copySrec(gpa, orig);
            insertNode(&(db->gpa_db), gpa, gpa_cmp);
        }
...
void putRecord(struct DB *db, char *buf)
{
    ...
    struct Srec *srec_lname = newSrec();
    ...
    setSrec(srec_lname, lname, initial[0], fname, strtoul(sid, NULL, 10), atof(gpa));
    printSrec(srec_lname);

    insertNode(&(db->lname_db), srec_lname, lname_cmp);
